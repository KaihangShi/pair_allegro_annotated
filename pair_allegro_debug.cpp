...
#pragma omp parallel for reduction(+:nedges)
  ...
  for(int ii = 1; ii < nlocal; ii++){
    ...
    printf("CHECKING neigh_Per_Atom, ii: %d, cumsum_ii: %d, cumsum_ii-1: %d, neigh_per_atom_ii_1: %d\n", 
    ii, cumsum_neigh_per_atom[ii], cumsum_neigh_per_atom[ii-1], neigh_per_atom[ii-1]);
  }
  ...
  for(int ii = 0; ii < ntotal; ii++){
    ...
    for(int jj = 0; jj < jnum; jj++){
      ...
      edges[1][edge_counter] = j;
						printf("ii %d, jj %d, ilist[ii] %d, jlist[jj] %d, ", ii, jj, ilist[ii], jlist[jj]);
						printf("itype %d, ij2type[i] %d, ", type[i], ij2type[i]);
      printf("i %d, j %d, edge_counter %d, itag-1 %d, jtag-1 %d, r %.10g\n", i, j, edge_counter, itag-1, jtag-1, sqrt(rsq));
      edge_counter++;
